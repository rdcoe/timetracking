...
- **DAOs** (`dao/`) use plain JDBC (no JPA/Hibernate — kept lightweight and
  ...
  one row per start/stop session.
- **`model/Project`** is a record; there is intentionally no `TimeEntry` model
  class because entries are only ever written, never read back into objects yet.
