...
- **`TrayNotifier`** is the only `java.awt` user (system-tray `TrayIcon`
  ...
  native config; worst case the notifier no-ops and the app still runs.
...
- **Daily Summary export.** The "Daily Summary" button writes one
  `daily-summary-<DATE>.json` (that day's per-project totals) for **every day from
  the last checkpoint through today**, then advances the checkpoint. Each project is
  tagged with a workstream inferred from its code (`xpq-eng*` → engineering,
  `xpq-sred*` → sred, else client). Output dir is `$XPQUEST_SUMMARY_DIR` if set,
  else `user.home/.xpquest` (OS-native: the same dir as the H2 file). JSON is
  hand-written — there is deliberately no JSON dependency.
  - The checkpoint is `.daily-summary-checkpoint` (a single date line) co-located
    with the JSON files. The app *reads* it for the range start (no file → earliest
    completed entry; no entries → today) and *writes* it back to today after
    generating. Days with no completed time are skipped; today's file is rewritten
    every press since its tracking is still in progress.
  - The app's only job is to emit those files. It has no knowledge of whatever may
    later read them; it is a standalone utility.
